...
#accel_int1_pin = machine.Pin(5, machine.Pin.IN) #IO5
#accel_int2_pin = machine.Pin(17, machine.Pin.IN) #IO17
...
def format_byte(val):
    rtn = "0b{}{}{}{}{}{}{}{}".format(
        1 if val & 0b10000000 else 0,
        1 if val & 0b01000000 else 0,
        1 if val & 0b00100000 else 0,
        1 if val & 0b00010000 else 0,
        1 if val & 0b00001000 else 0,
        1 if val & 0b00000100 else 0,
        1 if val & 0b00000010 else 0,
        1 if val & 0b00000001 else 0
    )
    return rtn
...
class BMX055:
    ...
    def init(self):
        ...
        if len(i2c_found) > 0:
            ...
            #accel_int1_pin.irq(trigger=machine.Pin.IRQ_RISING, handler=self._handle_accel_interrupt_1)
            #accel_int2_pin.irq(trigger=machine.Pin.IRQ_RISING, handler=self._handle_accel_interrupt_2)
    ...
    def mag_set_threshhold_low(self, val):
        data = struct.pack(FMT_SINGLE_BYTE, val)
        self._i2c.writeto_mem(self._addr_mag, 0x4F, data)
        return

    def mag_set_threshhold_high(self, val):
        data = struct.pack(FMT_SINGLE_BYTE, val)
        self._i2c.writeto_mem(self._addr_mag, 0x50, data)
        return
    ...
    def _configure_accel_interrupt_1(self):
        
        return

    def _configure_accel_interrupt_2(self):
        return

    def _configure_gyro_interrupt_1(self):
        return

    def _configure_gyro_interrupt_2(self):
        return

    def _configure_mag_drdy(self, enable=0x1, polarity=0x1):

        reg_4E_bits = self._i2c.readfrom_mem(self._addr_mag, 0x4E, 1)[0]
        toset = (polarity << 2) | (enable << 7)
        tokeep = ~toset

        reg_4E_bits = int((reg_4E_bits & tokeep) | toset)
        data = struct.pack(FMT_SINGLE_BYTE, reg_4E_bits)
        self._i2c.writeto_mem(self._addr_mag, 0x4E, data)

        return

    def _mag_interrupt_config_write(self, drdy_pin=None, int_pin=None, ch_z=None, ch_y=None, ch_x=None, dr_polarity=None, int_latch=None, int_polarity=None,
        overrun_en=None, overflow_en=None, high_z_en=None, high_y_en=None, high_x_en=None, low_z_en=None, low_y_en=None, low_x_en=None,
        threshold_high=None, threshold_low=None):

        reg_4E_bits = self._i2c.readfrom_mem(self._addr_mag, 0x4E, 1)[0]
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, drdy_pin, 7)
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, int_pin, 6)
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, ch_z, 5)
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, ch_y, 4)
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, ch_x, 3)
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, dr_polarity, 2)
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, int_latch, 1)
        reg_4E_bits = self._set_or_clear_bit(reg_4E_bits, int_polarity, 0)
        print("     reg_4E=%s" % format_byte(reg_4E_bits))

        data = struct.pack(FMT_SINGLE_BYTE, reg_4E_bits)
        print("       data=%s" % format_byte(data[0]))
        self._i2c.writeto_mem(self._addr_mag, 0x4E, data)

        reg_4D_bits = self._i2c.readfrom_mem(self._addr_mag, 0x4D, 1)[0]
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, overrun_en, 7)
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, overflow_en, 6)
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, high_z_en, 5)
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, high_y_en, 4)
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, high_x_en, 3)
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, low_z_en, 2)
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, low_y_en, 1)
        reg_4D_bits = self._set_or_clear_bit(reg_4D_bits, low_x_en, 0)
        print("     reg_4D=%s" % format_byte(reg_4D_bits))

        data = struct.pack(FMT_SINGLE_BYTE, reg_4D_bits)
        print("       data=%s" % format_byte(data[0]))
        self._i2c.writeto_mem(self._addr_mag, 0x4D, data)

        if threshold_low is not None:
            data = struct.pack(FMT_SINGLE_BYTE, int(threshold_low))
            print("threshold low data=%s" % format_byte(data[0]))
            self._i2c.writeto_mem(self._addr_mag, 0x4F, data)

        if threshold_high is not None:
            data = struct.pack(FMT_SINGLE_BYTE, int(threshold_high))
            print("threshold high data=%s" % format_byte(data[0]))
            self._i2c.writeto_mem(self._addr_mag, 0x50, data)

        return

    def _handle_accel_interrupt_1(self, pin):
        print("_handle_accel_interrupt_1 called...")
        return

    def _handle_accel_interrupt_2(self, pin):
        print("_handle_accel_interrupt_2 called...")
        return

    def _handle_gyro_interrupt_1(self, pin):
        print("_handle_gyro_interrupt_1 called...")
        return

    def _handle_gyro_interrupt_2(self, pin):
        print("_handle_gyro_interrupt_1 called...")
        return

    def _handle_mag_drdy(self, pin):
        print("_handle_mag_drdy called...")
        return
    ...
    def _set_or_clear_bit(self, reg_val, flag_val, flag_pos):
        if flag_val is not None:
            if flag_val:
                reg_val = reg_val | (0x1 << flag_pos)
            else:
                reg_val = reg_val & (~(0x1 << flag_pos))
        return reg_val